#!/usr/bin/env python3
"""
Скрипт для проверки заполненности товаров через API.
Проверяет последнюю инвентаризацию для указанных групп.
"""
...
# API URL
API_URL = "https://c8e767f0-ac37-4f85-88bd-7ce8bceb888c.selcdn.net/api/v1/inventory"
...
# Полный список товаров для проверки (все позиции из списка пользователя)
ITEMS_TO_CHECK = [
    # Напитки
...
def get_group_inventory(chat_id):
    """Получает инвентарь группы через API"""
    try:
        url = f"{API_URL}/{chat_id}"
        print(f"  🔗 Запрос: {url}")
        
        response = requests.get(url, timeout=10)
        
        if response.status_code == 404:
            print(f"  ⚠️  Группа не найдена")
            return None, None, None
        
        if response.status_code == 403:
            print(f"  ⚠️  Нет доступа к инвентарю (не chef группа)")
            return None, None, None
        ...
        chat_name = data.get('chat_title', 'Неизвестно')
        inventory = data.get('inventory', {})
        metadata = data.get('metadata', {})
        last_updated = metadata.get('lastUpdated', 'N/A')
        
        return chat_name, inventory, last_updated
...
def check_items_in_groups():
    """Основная функция проверки товаров по группам"""
    
    print("=" * 80)
    print("🔍 ПРОВЕРКА ВЫВЕДЕННЫХ ТОВАРОВ В ИНВЕНТАРИЗАЦИИ (через API)")
    print("=" * 80)
    ...
    for chat_id in CHAT_IDS:
        ...
        chat_name, inventory, last_updated = get_group_inventory(chat_id)
        ...
        if found_items:
            ...
            print(f"  ⚠️  Найдено {len(found_items)} заполненных товаров:")
